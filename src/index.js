...
class Purgecss {
  ...
  purge(): Array<ResultPurge> {
    ...
    return this.getCssContents(
      css,
      new Set([...cssFileSelectors, ...cssRawSelectors])
    );
  }
  ...
  extractRawSelector(
    ...
    for (const { raw, extension } of content) {
      ...
      selectors = new Set([
        ...selectors,
        ...this.extractSelectors(raw, extractor),
      ]);
    }
    return selectors;
  }
  ...
  isSelectorWhitelisted(selector: string): boolean {
    return (
      Boolean(CSS_WHITELIST.includes(selector)) ||
      !(
  ...
  isSelectorWhitelistedChildren(selector: string): boolean {
    return !!(
      ...
      this.options.whitelistPatternsChildren.some((v: RegExp) =>
        v.test(selector)
      )
    );
  }
